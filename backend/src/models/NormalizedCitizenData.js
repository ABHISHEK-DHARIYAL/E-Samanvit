 ...
 *      where it came from, when, and whether it's real ('verified') or
 *      demo ('mock') data, so a future UI can show e.g. "Annual Income
 *      ₹2,50,000 — Source: Revenue Department (Demo Data)" without the
 *      frontend having to guess or the app ever passing off a fallback
 *      as if it were a live government response.
 *
 ...
 * @param {string} [verificationStatus='mock'] - 'mock' for demo/prototype data,
 *   'verified' when the value actually came from a live, authenticated
 *   external call (e.g. a real Setu/DigiLocker fetch). Defaults to
 *   'mock' because every adapter in this prototype is mock-backed
 *   unless it explicitly proves otherwise by passing 'verified'.
 * @returns {object} partial normalized data, wrapped as
 *   { value, source, verifiedAt, verificationStatus }
 */
function selectRequiredFields(sourceRecord, requiredFields, sourceName, verificationStatus = 'mock') {
