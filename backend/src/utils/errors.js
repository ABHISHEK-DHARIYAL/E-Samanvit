...
/** A "Contact Us" submission is missing a required field or has an invalid value. */
class ContactValidationError extends AppError {
  constructor(fieldErrors) {
    super('One or more contact fields are invalid', 422, 'CONTACT_VALIDATION_FAILED');
    this.fieldErrors = fieldErrors; // { fieldName: message }
  }
}

/** The configured email provider (Resend) couldn't be reached or rejected the message. */
class EmailDeliveryError extends AppError {
  constructor(message) {
    super(message || 'Failed to send the email', 502, 'EMAIL_DELIVERY_FAILED');
  }
}

/**
 * A real external API (data.gov.in, Setu, India Post) failed or isn't
 * configured. This is never thrown out to a controller/citizen — every
 * caller of a real API catches this and falls back to mock data. It
 * exists purely so the fallback path has one consistent, loggable
 * error shape instead of each adapter inventing its own.
 */
class ExternalApiUnavailableError extends AppError {
  constructor(apiName, message) {
    super(`${apiName} unavailable: ${message}`, 502, 'EXTERNAL_API_UNAVAILABLE');
