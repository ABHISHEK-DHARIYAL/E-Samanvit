...
1. **The 4 real government systems** — Aaple Sarkar (identity/address),
   ...
   account). _None of these are actually connected yet — they're all
   represented by mock adapters that return realistic, fixed fake data._
...
The UI explains _why_ consent is needed and _which_ sources/fields will be
...
| **Schema-driven**    | The form's fields, labels, validation, and data sources are all _data_ (a schema file), not hardcoded UI code                                              |
