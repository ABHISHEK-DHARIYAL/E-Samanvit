...
const GovServices = {
  ...
  async onMount() {
    // A caller (e.g. a Students-category scheme card) may have asked
    // to jump straight to a specific service's consent step — see
    // App.navigate()'s 'gov-services' handling. Consumed once, then
    // cleared, so navigating back to this page later behaves normally.
    const preselectedServiceId = window.govServicesPreselectedServiceId;
    if (preselectedServiceId) {
      window.govServicesPreselectedServiceId = null;
      await this.selectService(preselectedServiceId);
      return;
    }
  ...
  async fetchFilledServiceIds() {
    try {
      ...
      const local =
        typeof FilledFormsCache !== "undefined"
          ? FilledFormsCache.getAll(GOV_DEMO_CITIZEN_ID)
          : [];
  ...
  async confirm() {
    ...
    try {
      ...
      // Mark this service as filled the moment submission succeeds,
      // so the Dashboard's Recommended/Filled split is correct
      // instantly if the citizen navigates there right after —
      // see FilledFormsCache.js's file comment for why this is a
      // cache, not a source of truth.
      if (typeof FilledFormsCache !== "undefined") {
        FilledFormsCache.markFilled(this.state.schema.serviceId, GOV_DEMO_CITIZEN_ID);
      }
