...
const GovServices = {
  ...
  stepBar() {
    const stepForKey = { list: 'service', consent: 'consent', preparing: 'autofill', form: 'autofill', review: 'review', done: 'review' };
    ...
    return `<div class="form-steps-bar" style="margin-bottom:var(--sp-6)">
      ${steps.map(([key, label]) => `<span class="step-pill ${key === active ? 'active' : ''}">${label}</span>`).join('')}
    </div>`;
  },
  ...
  renderBody() {
    ...
    switch (this.state.step) {
      ...
      case 'consent': return this.renderConsent();
      case 'preparing': return this.renderPreparing();
  ...
  renderList() {
    const cards = this.state.services.map((s) => `
      <div class="card card-hover">
        <div class="card-icon card-icon-blue">${Icons.fileText}</div>
        <div class="card-title">${escapeHtml(s.serviceName)}</div>
        <div class="form-hint" style="font-weight:600;color:var(--clr-primary-700);margin-bottom:6px">${escapeHtml(s.department)}</div>
        <p class="card-text">${escapeHtml(s.description)}</p>
        <button class="btn btn-primary btn-block" style="margin-top:var(--sp-3)" onclick="govSelectService('${escapeHtml(s.serviceId)}')">
  ...
  renderConsent() {
    ...
    const fieldLabels = schema.fields.filter((f) => f.commonDataPath).map((f) => f.label);
    const sourceLabels = [...new Set(schema.requiredSources.map(sourceLabel))];

    return `
      ${this.stepBar()}
      ${this.errorBanner()}
      <button class="btn btn-ghost btn-sm" onclick="govBackToList()">${Icons.chevronRight ? '←' : ''} Back to services</button>
      <div class="card" style="max-width:640px;margin:var(--sp-4) auto 0">
        ...
        <div style="margin-top:var(--sp-5)">
          ...
          <div style="display:flex;flex-wrap:wrap;gap:8px;margin-bottom:var(--sp-4)">
            ${fieldLabels.length
              ? fieldLabels.map((l) => `<span class="badge badge-blue">${Icons.check} ${escapeHtml(l)}</span>`).join('')
          ...
          <div style="display:flex;flex-wrap:wrap;gap:8px;margin-bottom:var(--sp-4)">
            ${sourceLabels.map((l) => `<span class="badge badge-green">${escapeHtml(l)}</span>`).join('')}
          </div>

          <div class="form-label" style="margin-bottom:4px">Purpose:</div>
          <p class="card-text" style="margin-bottom:var(--sp-3)">${escapeHtml(schema.purpose)}</p>
        ...
        <div style="display:flex;gap:var(--sp-3);margin-top:var(--sp-6)">
          ...
          <button class="btn btn-primary btn-block ${this.state.loading ? 'btn-loading' : ''}" ${this.state.loading ? 'disabled' : ''} onclick="govGiveConsent()">
    ...
    return `
      ...
      <div style="text-align:center;padding:var(--sp-10) 0">
        ...
        <p class="form-hint" style="margin-top:var(--sp-3);font-weight:600">
          ${this.state.consentToken ? 'Fetching authorized data and preparing your application...' : 'Creating secure consent...'}
        </p>
      </div>
    `;
  },
  ...
  renderForm() {
    const schema = this.state.schema;
    const rows = schema.fields.map((f) => FieldRenderer.row(this.state.fields[f.fieldId], this.state.formErrors[f.fieldId])).join('');
    ...
    return `
      ...
      <div class="card" style="max-width:640px;margin:var(--sp-4) auto 0">
        ...
        <form onsubmit="event.preventDefault();govGoToReview();">
          ${rows}
          <button type="submit" class="btn btn-primary btn-block" style="margin-top:var(--sp-3)">Continue to Review ${Icons.arrowRight}</button>
        </form>
      </div>
    `;
  },
  ...
  renderReview() {
    const schema = this.state.schema;
    const rows = schema.fields.map((f) => FieldRenderer.reviewRow(this.state.fields[f.fieldId])).join('');
    ...
    return `
      ...
      <div class="card" style="max-width:640px;margin:0 auto">
        ...
        <p class="form-hint" style="margin-bottom:var(--sp-4)">${escapeHtml(schema.serviceName)} — please check everything below before confirming.</p>
        ${rows}
        ...
        <div style="display:flex;gap:var(--sp-3);margin-top:var(--sp-5)">
          <button class="btn btn-secondary btn-block" ${this.state.submitting ? 'disabled' : ''} onclick="govBackToForm()">Back &amp; Edit</button>
          <button class="btn btn-primary btn-block ${this.state.submitting ? 'btn-loading' : ''}" ${this.state.submitting ? 'disabled' : ''} onclick="govConfirm()" aria-busy="${this.state.submitting ? 'true' : 'false'}">
  ...
  renderDone() {
    ...
    return `
      ...
      <div class="card" style="max-width:640px;margin:0 auto;text-align:center">
        <div class="card-icon card-icon-green" style="margin:0 auto">${Icons.check}</div>
        ...
        ${app ? `
          <div class="card" style="background:var(--clr-bg-subtle,#f6f8fa);margin-top:var(--sp-4);text-align:left">
            ...
            <p class="card-text" style="font-family:monospace;word-break:break-all">${escapeHtml(app.applicationId)}</p>
  ...
  async onMount() {
    if (this.state.step === 'list' && this.state.services.length === 0 && !this.state.loading) {
  ...
  deriveRequirements() {
    const schema = this.state.schema;
    const fields = [...new Set(schema.fields.map((f) => f.commonDataPath).filter(Boolean))];
  ...
  async giveConsent() {
    ...
    this.state.loading = true; this.state.error = null; this.state.step = 'preparing'; this.refresh();
    try {
  ...
  async prepareService() {
    try {
      ...
      for (const field of this.state.schema.fields) {
        const mapped = res.fields[field.fieldId] || { value: null, autoFilled: false, missing: true, source: null };
  ...
  goToReview() {
    const errors = validateAllFields(this.state.schema.fields, this.valuesOnly());
    this.state.formErrors = errors;
    if (Object.keys(errors).length > 0) { this.refresh(); return; }
  ...
  valuesOnly() {
    const out = {};
    for (const [id, f] of Object.entries(this.state.fields)) out[id] = f.currentValue;
    return out;
  },
  ...
  async confirm() {
    ...
    try {
      ...
      if (e.code === 'APPLICATION_INCOMPLETE' || e.code === 'APPLICATION_VALIDATION_FAILED') {
        const errors = {};
        for (const fieldId of e.missingFields || []) errors[fieldId] = 'This field is required.';
        Object.assign(errors, e.fieldErrors || {});
        this.state.formErrors = errors;
        this.state.step = 'form';
        this.state.error = 'A few things need fixing before this can be submitted.';
      } else {
        this.state.error = friendlyErrorMessage(e);
      }
    } finally {
...
function friendlyErrorMessage(err) {
  ...
  const byCode = {
    CONSENT_TOKEN_MISSING: 'Your consent has expired. Please provide consent again.',
    CONSENT_TOKEN_INVALID: 'Your consent has expired. Please provide consent again.',
    ...
    CONSENT_SCOPE_VIOLATION: 'This service requires information that is not included in your current consent.',
    CONSENT_CITIZEN_MISMATCH: 'This service requires information that is not included in your current consent.',
    SERVICE_NOT_FOUND: 'This service is temporarily unavailable.',
    INVALID_REQUEST: 'Something about this request was incomplete. Please try again.',
    APPLICATION_NOT_FOUND: 'That application could not be found.'
  };
  return (err.code && byCode[err.code]) || 'Something went wrong. Please try again.';
}

// -- Global handlers wired from onclick/oninput in the templates above --
function govSelectService(id) { GovServices.selectService(id); }
function govBackToList() { GovServices.backToList(); }
function govBackToForm() { GovServices.backToForm(); }
function govGiveConsent() { GovServices.giveConsent(); }
function govUpdateField(id, value) { GovServices.updateField(id, value); }
function govGoToReview() { GovServices.goToReview(); }
function govConfirm() { GovServices.confirm(); }
function govRetry() { GovServices.retry(); }
