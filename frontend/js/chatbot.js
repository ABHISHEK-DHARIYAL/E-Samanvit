...
You can also explore our **Services** section for the full directory.`;
  ...
  processMessage(userText) {
    ...
    // 3. Match Query against Knowledge Base first — this is fast,
    // deterministic, already localized in 3 languages, and works with
    // zero backend/network dependency. Gemini (if configured) is only
    // ever consulted for what this doesn't already answer.
    ...
    if (matched) {
      setTimeout(() => {
        ...
        this.appendMessage('bot', matched.response[lang] || matched.response['en'], matched.action || null);
      }, 600);
      return;
    }

    this.askGeminiOrFallback(userText, lang);
  },

  // Only reached when the local knowledge base found no match. Tries
  // the backend's Gemini proxy; on ANY non-answer (not configured,
  // network failure, timeout, empty reply) it falls back to the
  // existing getFallbackResponse() exactly as before this feature
  // existed — MITRA's core behavior never depends on this succeeding.
  async askGeminiOrFallback(userText, lang) {
    let reply = null;
    try {
      const data = await ApiClient.request('/api/mitra/ask', {
        method: 'POST',
      ...
      // Backend unreachable, timed out, or errored — silently fall
      // through to the existing scripted fallback below.
    }

    this.setTyping(false);
    this.appendMessage('bot', reply || this.getFallbackResponse(userText));
  },
